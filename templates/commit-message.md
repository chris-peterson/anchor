...
`anchor.commit.rules` adds a standing rule to every message — e.g.
`git config anchor.commit.rules "prefix the subject with the affected module"`.

`anchor.commit.verbosity` (1–100, unset behaves as `50`) decides how much prose
...
CR-side `cr.rules` / `gitlab.rules` / `github.rules`).
