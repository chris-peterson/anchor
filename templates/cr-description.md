...
what each is for. The `cr` skill owns the *technique* for realizing
...
routing the reviewer: *what to look for*, or *why this change matters*. If the
only thing it would say is *what changed*, drop the clause and leave the bare
link. The reviewer is one click from the change; restating it is the "described
the *what*" failure. Never expand a bullet into numbered prose steps that walk
the edits ("remove the X block", "drop the Y key") — the deep-linked change
already shows them.
...
(see the `cr` skill, Step 3 "What to avoid").
...
`cr` resolves one before drafting, most specific level first — the
