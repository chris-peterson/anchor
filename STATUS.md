...
**Coverage:** 254 Covered, 10 Partial, 2 Contradicts
...
| HOST-01..07 | 7 | All Covered | Cross-host runtime and packaging — neutral `<anchor-root>` paths and capability language in every `skills/*/SKILL.md`, resolved by `guides/host-runtime.md`; dual Claude Code/Codex invocation docs in `docs/README.md`; both invocation forms excluded from deep-link placeholders by `scripts/deep-links.sh`; portable `plugin.json` alongside `.claude-plugin/plugin.json`; portable/compatibility manifest ordering in `scripts/release-recon.sh`; portable and Claude hook roots in `hooks/{hooks.yml,emit-rules.sh}`; and the explicit POSIX/Git Bash/WSL2 boundary in `guides/temp-paths.md`. `tests/{plugin-root-refs,deep-links,release-recon}.test.sh` hold the contracts. |
