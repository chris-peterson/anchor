...
**Coverage:** 259 Covered, 10 Partial, 2 Contradicts
...
| HOST-01..12 | 12 | All Covered | Cross-host runtime and packaging — neutral `<anchor-root>` paths and capability language in every `skills/*/SKILL.md`, resolved by `guides/host-runtime.md`; dual Claude Code/Codex invocation docs in `docs/README.md`; both invocation forms excluded from deep-link placeholders by `scripts/deep-links.sh`; portable `plugin.json` alongside `.claude-plugin/plugin.json`; portable/compatibility manifest ordering in `scripts/release-recon.sh`; portable and Claude hook roots in `hooks/{hooks.yml,emit-rules.sh}`; the explicit POSIX/Git Bash/WSL2 boundary in `guides/temp-paths.md`; and reversible developer pinning in `dev/{pin-local,unpin-local}.sh`. `tests/{plugin-root-refs,deep-links,release-recon,dev-local-pin}.test.sh` hold the contracts. Bounded entry points route to required `skills/*/references/*.md` phases; `guides/reading-instructions.md` governs complete phase and targeted shared-guide reads. `scripts/check-skill-context.sh` and `tests/skill-context.test.sh` enforce byte budgets and phase reachability, and the config/default and temp-path checks scan nested references (HOST-09..12). |
...
### 2026-09-21 — Complete skill workflows fit the host prompt budget (HOST-09..12)

STATUS.md updated: +4 IDs (HOST-09..12, Covered), 267 → 271.

All nine entry points retain workflow routing and essential gates below a
6,000-byte budget. The original procedures, examples, and diagrams moved intact
into linked phase files below 8,000 bytes, read in full before that phase runs.
Existing coverage rows naming a skill's steps now refer to its linked phase
files as well as its entry point; requirement coverage did not shrink with the
entry points. The same references are projected into the docs site.

The shared runtime requires bounded, complete reads and the targeted sections
in `guides/reading-instructions.md`. Missing and unlinked references and byte
budget violations have failure fixtures; existing default-value and temp-path
checks include nested phase files. Phase relocation was checked by reconstructing
each original body in order and comparing every byte apart from file-boundary
blank lines.

### 2026-09-21 — The checkout can replace and restore both host installs (HOST-08)

STATUS.md updated: +1 ID (HOST-08, Covered), 266 → 267.

`dev/pin-local.sh` removes installed Anchor marketplace copies, loads the
checkout in place through Claude Code's skills directory, and installs a
checkout snapshot through a Codex local marketplace. `dev/unpin-local.sh`
removes those local forms, refreshes the canonical marketplace, and reinstalls
its current Anchor release. Either host may be absent without its configuration
paths being touched. The scripts preserve Claude plugin data, reject a
developer-owned path rather than replacing it, and are held against isolated
fake host state by `tests/dev-local-pin.test.sh`.

### 2026-09-18 — Claude Code and Codex share one host-neutral workflow (HOST-01..07)

STATUS.md updated: +7 IDs (HOST-01..07, all Covered), 259 → 266.

Skills now resolve bundled files without assuming a hook-only environment
variable, name capabilities instead of Claude Code tool names, and hand off by
neutral skill name. The package keeps its Claude manifest while adding the
portable Agent Plugins manifest, the hook accepts either host's root variable,
release reconnaissance recognizes every supported manifest location, and the
docs show each host's invocation and Windows shell boundary.
