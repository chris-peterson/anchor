...
- **DIFF-24 (Partial)** — the requirement holds in `skills/{issue,release}/SKILL.md`. `skills/prepare-review/SKILL.md` probes under its own skill but names the tool it opens only when the probe reports `REVIEW_BACKEND_CONFIGURED`, so its defaulted `editor` still opens unannounced; `skills/commit/SKILL.md` skips the probe entirely. Extending the rule to both, or narrowing DIFF-24 to the skills that draft a standalone document, is a decision pending.
