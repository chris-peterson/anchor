...
#
#   scripts/review/modes/edit.sh    opens the artifact, and needs the host to open it in
...
# shellcheck source=review-host.sh
source "$(dirname "${BASH_SOURCE[0]}")/review-host.sh"
# shellcheck source=editor-flags.sh
source "$(dirname "${BASH_SOURCE[0]}")/editor-flags.sh"
...
# preferred first. Kept to the editors that take `--wait`; anything else is a
# `core.editor` away (DIFF-16).
...
# iTerm2 session's split. Asked of the host selector with
# no editor named, so resolution and launch cannot disagree about it, and so the
# `gui` host — a window rather than a terminal — cannot answer for it.
...
# can only render inside a terminal some host puts up.
#
# The hosts and their order are the dispatcher's (scripts/lib/review-host.sh), and
# asking it rather than repeating them is what keeps `edit` reaching exactly as
# far as `diff` does. An explicit launcher still wins: it is the escape hatch for
# a host anchor does not ship, and the seam the tests drive, and unlike the rest
# it takes the file rather than a command string — so `edit` opens it itself.
anchor_editor_host() {
  if [[ -n "${ANCHOR_EDITOR_LAUNCHER:-}" ]]; then printf 'launcher'; return; fi
  anchor_review_host edit "${1:-}"
}
