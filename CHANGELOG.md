...
> ```
>
> Set `anchor.diff.tool` even if `revdiff` is what you already had. Absent it, a
> `diff` review falls through to git's own `diff.tool` — so a `diff.tool` you set
> long ago for `git difftool` is what opens, and `revdiff` runs only when neither
> key is set.
