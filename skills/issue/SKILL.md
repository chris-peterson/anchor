...
`--get-regexp` lowercases the section and the setting (`anchor.cr.templateRepo` comes back as `anchor.cr.templaterepo`) and leaves the qualifier as written; match the first two case-insensitively and the qualifier exactly. Apply the keys relevant to an issue; absent keys keep `anchor`'s defaults — never invent a value:
...
- **`anchor.issue.rules`** — an extra standing rule layered onto every issue (the escape hatch for anything without a dedicated key).
- **`anchor.issue.verbosity`** — an integer 1–100 setting where the issue body sits between brevity and thoroughness. **Unset behaves as `75`** — the highest of `anchor`'s four verbosity defaults, which descend as the audience widens (issue `75` → commit `50` → CR `25` → release `10`). An issue's audience is the few people who'll do the work, and background that would pad a release note saves them a conversation here. **It is not a word budget** — nothing is counted or truncated. Clamp an out-of-range or non-integer value into the 1–100 band and say so once rather than failing the draft.
