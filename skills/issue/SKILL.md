...
Ungraded for any reason — nothing installed, or a review that came back without a verdict — walks the ladder in `${CLAUDE_PLUGIN_ROOT}/guides/review-fallback.md`: link the draft file, offer the editor rung when the probe reported `REVIEW_EDITOR_AVAILABLE=1`, and otherwise put the full body in a fenced block and ask what to change.
