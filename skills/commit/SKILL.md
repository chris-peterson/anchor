...
- **`no-verdict`** → nothing is committed; read the cause from the result, say what happened in one line, and walk the fallback ladder in `${CLAUDE_PLUGIN_ROOT}/guides/review-fallback.md`. `backend: "difftool"` (or `capabilities.producesVerdict: false`) means the user selected `anchor.reviewBackend git` and their difftool showed the change — it grades nothing, so the ladder applies to it exactly as it does to a backend that died (see `raw.exitCode`). **Never ask `Reviewed in your difftool — commit and push?`**: a launched window is not evidence anything was read, and treating it as approval is what the verdict exists to prevent. Here the ladder's changeset rung is the one to walk — go file by file over the pending changeset in your reply.
