...
- **`no-verdict`** → nothing is committed; read the cause from the result, say what happened in one line, and walk the fallback ladder in `${CLAUDE_PLUGIN_ROOT}/guides/review-fallback.md`. `capabilities.producesVerdict: false` means the backend graded nothing, so the ladder applies to it exactly as it does to a backend that died (see `raw.exitCode`). **Never ask `Reviewed in your diff viewer — commit and push?`**: a launched window is not evidence anything was read, and treating it as approval is what the verdict exists to prevent. Here the ladder's changeset rung is the one to walk — go file by file over the pending changeset in your reply.
...
**The message is under review too.** If the result carries `editedFields` for the commit message — the `editor` backend, where the saved buffer *is* the message — use that edited text as the message in Step 6, overwrite the Step 3 message file with it. Adopt it verbatim: it's the user's own wording, not a comment to draft from. A `changes-requested` comment with `target: "commit-message"` is feedback on the message itself — revise the message and re-review. On a backend that can't round-trip an edited message (`capabilities.editableCommitMessage: false`, e.g. revdiff), keep the drafted message unless a comment asks to change it.
