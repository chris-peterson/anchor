...
- `Backticks` for every code identifier, file path, env var, branch name, package, or CLI flag. Anything a reader might grep for or paste into a terminal earns backticks. Inline backticks are cheap and pay back in skim-readability.
  ...
  - **Cross-project references need the full URL.** A bare `#42` / `!148` / `@name` shortcut resolves *within the CR's own project* — `#42` autolinks to issue 42 of that project, not to a same-numbered item elsewhere. A pipeline, issue, or CR in a different project (CI/deploy pipelines routinely live in their own) must be written as a full URL; the bare shortcut silently links the wrong target and the mistake is invisible in the markdown source — it surfaces only once rendered. See the [forge cookbook](/guides/forge-cookbook) for the full GitLab-markdown facts.
